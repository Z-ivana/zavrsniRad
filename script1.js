...
let lStorage = (function () {
    let loadFromLocalStorage = function (nameOfObject) {
        let object = localStorage.getItem(nameOfObject);
...
let logRegForget = function () {
    let log = (function () {
        if (!lStorage("ulogovanKorisnik").load) return false;
        return true;
    })();
    ...
    let allUsers = lStorage("allUsers").load;
...
let booksAndAutors = function () {
    ...
    let allBoks = lStorage("allBooks").load;
    if (allBooks == null) allBooks = {};

    let allAuthor = lStorage("allAuthor").load;
